...
public interface AccountRepository extends CrudRepository<Account, Integer> {
    ...
    double getAccountBalance2(@Param("account_number") int account_number);
    ...
    //NEW ADDED FOR UPDATING BALANCE BY ACCOUNT NUMBER
    @Modifying
    @Query(value ="UPDATE accounts SET balance = :new_balance WHERE account_number = :account_number " , nativeQuery = true)
    @Transactional
    void changeAccountBalanceByAccountNumber(@Param("new_balance") double new_balance, @Param("account_number") int account_number);
